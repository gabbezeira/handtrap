...
export const HandSimulationModal: React.FC<HandSimulationModalProps> = ({ isOpen, onClose, deck, extraDeck }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(doc(db, 'users', user.uid, 'usage', 'hand_analysis'), async (docSnap) => {
            if (docSnap.exists()) {
    ...
    return createPortal(
        ...
        <Overlay onClick={onClose}>
            <ModalContent onClick={e => e.stopPropagation()}>
                ...
                <Header>
                    <h3><DicesIcon><Dices size={24}/></DicesIcon> Simulação de Mão Inicial</h3>
                    <CloseButton onClick={onClose}><X size={24}/></CloseButton>
                </Header>
                <Body>
                  {deck.length < 5 ? (
                     <DeckMinMessage>O deck precisa de pelo menos 5 cartas para simular uma mão.</DeckMinMessage>
                  ) : (
                    <>
                        <HandGrid>
                            {hand.map((card, idx) => (
