...
export const HandAnalysisResultModal: React.FC<HandAnalysisResultModalProps> = ({ isOpen, onClose, analysis }) => {
    ...
    <S.AnalysisOverlay onClick={onClose}>
      <S.AnalysisContent onClick={e => e.stopPropagation()}>
        <S.AnalysisHeader>
          ...
          <S.AnalysisCloseBtn onClick={onClose}><X size={18} /></S.AnalysisCloseBtn>
        </S.AnalysisHeader>
        ...
        <S.AnalysisScrollableBody>
          ...
          <S.ScoreSection>
             ...
             <S.ScoreText>
                <h4>
                    {analysis.score >= 8 ? 'Mão Excelente' : analysis.score >= 5 ? 'Mão Jogável' : 'Mão Fraca / Brick'}
                </h4>
                <p>
                    {analysis.score >= 8 
          ...
          <S.GridSection>
             <S.StrategyCard $variant="first">
                <h3><PlayCircle size={18}/> Going First</h3>
                <p>{analysis.strategy_going_first}</p>
             </S.StrategyCard>
             <S.StrategyCard $variant="second">
                <h3><S.RotatedIcon><PlayCircle size={18}/></S.RotatedIcon> Going Second</h3>
          ...
          {analysis.key_combos && analysis.key_combos.length > 0 && (
              <S.CombosSection>
                 <S.SectionTitle><Trophy size={18}/> Combos Chave</S.SectionTitle>
