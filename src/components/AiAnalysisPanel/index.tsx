...
export const AiAnalysisPanel: React.FC<AiAnalysisPanelProps> = ({
  ...
  const submitFeedbackVote = async () => {
    ...
    try {
      ...
      setFeedbackError(
        error.response?.data?.message || 'Erro ao enviar feedback. Tente novamente.',
      );
    } finally {
  ...
  return (
    <S.PanelContainer>
            ...
            {usageLimit && (
              <S.UsageCredits $isLimitReached={isLimitReached}>
                {isLimitReached
                  ...
                  : `Uso Hoje: ${usageLimit.used}/${usageLimit.limit}`}
              </S.UsageCredits>
            )}
          </S.UsageText>
        </S.EmptyStateContainer>
      ...
      {isLoading && <AiLoader message="Analisando Estratégias do Deck..." />}
      ...
      {result && !isLoading && (
        <S.ResultsContainer>
            ...
            <S.CommunityBanner>
              ...
              {!hasVoted && (
                <S.FeedbackContainer>
                  ...
                  <S.FeedbackButtons>
                    <S.VoteButton $type="accurate" onClick={() => handleVote('accurate')}>
                      <ThumbsUp size={14} /> Sim
                    </S.VoteButton>
                    <S.VoteButton $type="inaccurate" onClick={() => handleVote('inaccurate')}>
                      <ThumbsDown size={14} /> Não
                    </S.VoteButton>
                  </S.FeedbackButtons>
                </S.FeedbackContainer>
              )}
              {hasVoted && <S.FeedbackSuccess>✓ Obrigado pelo seu feedback!</S.FeedbackSuccess>}
            </S.CommunityBanner>
          )}
          ...
          <div>
            <S.SectionHeader>
              <Shield size={16} /> Meta Matchups
            </S.SectionHeader>
            <S.MatchupsGrid>
              {result.matchups?.map((m, i) => (
                <S.MatchupCard key={i}>
                  <S.MatchupDeckName>{m.deckName}</S.MatchupDeckName>
                  <S.MatchupWinRate $winRate={m.winRate}>{m.winRate}% VR</S.MatchupWinRate>
          ...
          <S.ScoreGrid>
            <ScoreBar
              ...
              value={result.metaScore.poderOfensivo}
              color="#ef4444"
              icon={Zap}
            />
            <ScoreBar
              label="Consistência"
              value={result.metaScore.consistencia}
              color="#eab308"
              icon={Target}
            />
            <ScoreBar
              label="Resiliência"
              value={result.metaScore.resiliencia}
              color="#3b82f6"
              icon={Shield}
            />
            <ScoreBar
              label="Controle"
              value={result.metaScore.controle}
              color="#a855f7"
              icon={Lock}
            />
          </S.ScoreGrid>
          ...
          <div>
            <S.SectionHeader>
              <BrainCircuit size={16} /> Análise Geral
            </S.SectionHeader>
          ...
          <S.GamePlanGrid>
            <S.GamePlanCard $mode="turn1">
              ...
              <S.PlanDescription>{result.planoDeJogo?.turno1 || 'N/A'}</S.PlanDescription>
            </S.GamePlanCard>
            <S.GamePlanCard $mode="turn2">
              <S.PlanTitle $color="#f87171">Turno 2 (Going Second)</S.PlanTitle>
              <S.PlanDescription>{result.planoDeJogo?.turno2 || 'N/A'}</S.PlanDescription>
            </S.GamePlanCard>
          </S.GamePlanGrid>
            ...
            <div>
              ...
              <S.PointList>
                {result.pontosFracos.map((p, i) => (
                  <S.PointItem key={i} $type="bad">
                    <ThumbsDown size={14} /> {p}
                  </S.PointItem>
                ))}
              </S.PointList>
            </div>
          ...
          <div>
            <S.SectionHeader>
              <Layers size={16} /> Combos Principais
            </S.SectionHeader>
      ...
      {showFeedbackModal && (
        <S.FeedbackModalOverlay>
          <S.FeedbackModalContent>
            ...
            />
            {feedbackError && <S.FeedbackErrorMessage>{feedbackError}</S.FeedbackErrorMessage>}
            <S.FeedbackActions>
              <S.CancelButton
                onClick={() => {
                  setShowFeedbackModal(false);
