...
export const AdminDashboard = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <DashboardContainer>
      <Header>
        <Title>
          <Activity size={24} /> Monitoramento de API
        </Title>
        <CloseButton onClick={onClose}>
          <X size={20} />
        </CloseButton>
      ...
      <StatsGrid>
        ...
        <StatCard $type="cost">
          <CardTitle>
            <DollarSign size={20} color="#10b981" /> Custos Gemini API (30 dias)
          </CardTitle>
          ...
          {loadingUsage ? (
            ...
            <>
              ...
              {usageStats.byOperation &&
                Object.entries(usageStats.byOperation).map(([op, data]) => (
                  <MetricRow key={op}>
                    <OperationLabel>
                      {op}: {data.calls}x
                    </OperationLabel>
        ...
        <StatCard $type="system">
          <CardTitle>
            <Server size={20} color="#3b82f6" /> Backend (System)
          </CardTitle>
          ...
          <MetricRow>
            ...
            <MetricValue>
              {systemMetrics.requests
                ? Math.round(systemMetrics.totalLatency / systemMetrics.requests)
                : 0}
              ms
            </MetricValue>
          </MetricRow>
        ...
        <StatCard $type="external">
          <CardTitle>
            <Globe size={20} color="#ec4899" /> Yu-Gi-Oh API (External)
          </CardTitle>
          ...
          <MetricRow>
            <span>Latência Média</span>
            <MetricValue>
              {externalMetrics.requests
                ? Math.round(externalMetrics.totalLatency / externalMetrics.requests)
                : 0}
              ms
            </MetricValue>
          </MetricRow>
