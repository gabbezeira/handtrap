...
export const getDeckAnalysisUsage = async (userId: string): Promise<{ used: number; limit: number }> => {
    return getUsageCheck(userId, 'deck_analysis', 'deck');
};

export const getHandAnalysisUsage = async (userId: string): Promise<{ used: number; limit: number }> => {
    return getUsageCheck(userId, 'hand_analysis', 'hand');
};

export const getCardAnalysisUsage = async (userId: string): Promise<{ used: number; limit: number }> => {
    return getUsageCheck(userId, 'card_analysis', 'card');
};

const getUsageCheck = async (userId: string, collectionName: string, type: 'deck' | 'hand' | 'card') => {
    try {
