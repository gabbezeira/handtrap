...
export const SavedDecks = () => {
  ...
  const handleImportDeck = async () => {
    ...
    try {
      ...
      const resolveCards = (ids: number[]) =>
        ids.map((id) => cardMap.get(id)).filter(Boolean) as CardData[];
      ...
      showModal({
        title: 'Sucesso',
        message: 'Deck importado e salvo na sua conta com sucesso!',
        type: 'success',
      });
      loadDecks();
    } catch (error: any) {
      ...
      showModal({
        ...
        message: 'Link inválido ou erro ao processar: ' + error.message,
        type: 'error',
      });
    } finally {
    ...
    <PageWrapper>
      ...
      <Container>
        <TitleSection>
          ...
          <ActionsWrapper>
            <ImportWrapper>
              ...
              />
              <ImportButton onClick={handleImportDeck} disabled={!importLink || isImporting}>
                {isImporting ? (
                  <Calendar className="animate-spin" size={16} />
                ) : (
                  <Download size={16} />
                )}
                Importar
        ...
        <WarningBanner>
          ...
          <span>
            <strong>Aviso:</strong> O sistema de banlist está em construção. No momento, não há
            restrições automáticas para cartas banidas ou limitadas ao adicionar ao deck.
          </span>
        </WarningBanner>
